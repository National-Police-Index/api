...
# Put the repo root on sys.path so the `resolve` / `shared` packages import regardless of
# cwd. (The new resolve package resolves its model/data files via __file__, so no chdir.)
...
class PipelineScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(id="body"):
            ...
            with Vertical(id="out"):
                yield Label("Candidates — gate columns: ✓ passed · ⚑ flagged · ✗ failed · · n/a",
                            classes="pane-title")
    ...
    def on_mount(self) -> None:
        ...
        # mention-level gates (common, uniq, ambig) + per-candidate gates (thr, exact, best, agency)
        t.add_columns("candidate", "prob", "common", "uniq", "thr", "exact", "ambig",
                      "best", "agency", "post agency")
        style_table(t)
        self.query_one("#first", Field).focus_input()
    ...
    def _show_pipeline(self, result, log, err):
        ...
        if result.get("status") == "auto_matched" and match:
            self._verdict(
                f"[b green]AUTO-MATCHED[/b green] → {match['post_first_name']} {match['post_last_name']} "
                f"| {match['post_agency_name']} | POST {match['post_person_nbr']} "
                f"| prob {match['match_probability']:.3f}"
            )
        else:
            extra = " [b red](AMBIGUOUS)[/b red]" if result.get("ambiguous") else ""
            self._verdict(
                f"[b yellow]ROUTED TO REVIEW[/b yellow]{extra} — {result.get('reason') or 'No match'}"
            )

        # Glyphs for every gate cell.
        G = {"pass": "[green]✓[/green]", "flag": "[yellow]⚑[/yellow]",
             "fail": "[red]✗[/red]", "skip": "[dim]·[/dim]", None: "[dim]·[/dim]"}

        def cmark(v):  # per-candidate bool -> glyph (None = not evaluated)
            return G["pass"] if v is True else (G["fail"] if v is False else G["skip"])

        # Mention-level gate statuses (constant across candidate rows), keyed by gate name.
        gmark = {g["name"]: g["status"] for g in (result.get("gates") or [])}
        common = G[gmark.get("Common last name", "skip")]
        uniq = G[gmark.get("Unique name in state", "skip")]
        ambig = G[gmark.get("Not ambiguous", "skip")]

        t = self.query_one("#cands", DataTable)
        t.clear()

        # Flatten the gate sections (already ordered best-first) into candidate rows.
        cands = [c for sec in (result.get("sections") or []) for c in sec["candidates"]]
        for c in cands:
            prob = c.get("match_probability")
            t.add_row(
                f"{c['post_first_name']} {c['post_last_name']} ({c['post_person_nbr']})",
                f"{prob:.3f}" if prob is not None else "",
                common, uniq,
                cmark(c.get("above_threshold")),
                cmark(c.get("exact_name")),
                ambig,
                cmark(c.get("is_best")),
                cmark(c.get("agency_valid")),
                c["post_agency_name"],
            )

        # Early-stop cases generate no candidates — show one row for the mention itself so the
        # flagged gate (e.g. common last name) is still visible in the table.
        if not cands:
            m = result.get("mention") or {}
            t.add_row(
                f"{m.get('first', '')} {m.get('last', '')}".strip() or "(mention)",
                "", common, uniq, G["skip"], G["skip"], ambig, G["skip"], G["skip"],
                "[dim]no candidates generated[/dim]",
            )
