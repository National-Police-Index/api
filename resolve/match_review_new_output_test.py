...
if __name__ == "__main__":
    ...
    results, all_candidates, invalid_candidates, full_employment_histories = matcher.find_canonical_stint(mentions)
    ...
    output_df.to_csv("data/output/df_V2.csv", index=False)
    ...
    if len(auto_match_results) > 0:
        ...
        for idx, (_, match) in enumerate(auto_match_results.iterrows()):
            ...
            if mention_uid in full_employment_histories:
                ...
                # Fetch ALL officers with same name from entire database (for manual review)
                all_name_candidates = pd.DataFrame()
                print(f"DEBUG: Searching entire database for '{first_name} {last_name}'")
        ...
        if len(conflicts_sheets) > 0:
            with pd.ExcelWriter("data/output/matched_employment_histories_v2.xlsx", engine="openpyxl") as writer:
        ...
        if len(clean_sheets) > 0:
            with pd.ExcelWriter("data/output/matched_clean_no_conflicts_v2.xlsx", engine="openpyxl") as writer:
            ...
            jsonl_output_path = "data/output/matched_clean_no_conflicts_v2.jsonl"
